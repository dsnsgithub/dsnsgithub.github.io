...
async function getStatus(playerName) {
	let response = await (
		await fetch(
			`https://api.slothpixel.me/api/players/${playerName}/status`
		)
	).json();
...
async function getRecentGame(playerName) {
	let response = await (
		await fetch(
			`https://api.slothpixel.me/api/players/${playerName}/recentGames`
		)
	).json();
	
	time = new Date(response[0]["date"]).toString("en-US", {
		hour: "numeric",
		minute: "numeric",
		hour12: true,
	});
